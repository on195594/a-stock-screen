...
| **第 2 层：交互与线框** | 信息依据、结果状态、个人研究闭环 | [§2 Web端交互与 UI（兼容移动端）](#2-web端交互与-ui兼容移动端) | S1 扫描看得懂 / S2 持续研究 |
| **第 3 层：系统架构与持久化** | Web+Worker 进程模型、SQLite 3张表、快照导入 | [§3 架构和现有代码接入](#3-架构和现有代码接入) · [§4 存储合同](#4-存储合同) · [§5 更新与恢复](#5-更新数据含义和失败恢复) | 复用现有工作区 / S2 watch 任务 |
| **第 4 层：认证与运维** | GitHub OAuth、Nginx 代理、Docker Compose | [§6 认证、Web与移动端重连与云端部署](#6-认证web与移动端重连与云端部署) · [§7 Codex 与 AI 兼容](#7-codex-与-ai-兼容) | 持续保留鉴权 / S3 实际使用验收 |
...
| 部署 | Nginx HTTPS/WSS → 单个 Flet/ASGI 网页进程；Compose 管理 Web 与独立 worker，共用镜像、分配角色凭据 |
...
后台 worker 只是执行**用户已经提交的任务**，不是自动发起投资研究。备份维护不等于定时选股。复用现有三表、worker、认证和算法；本轮不以合并容器、换框架、增加因子或AI总结替代产品流程修复。
...
当前缺口主要是**工程操作可执行，研究任务未闭合**，不是缺少更多后台模块。以下为 `9a78ff3` 源码审查所得，不是生产登录、加载耗时或真机实测：

| 用户问题 | 当前缺口与源码依据 | 落地切片 |
|---|---|---|
| 哪些公司值得继续核查，为什么？ | `app.py:render_discover` 主要显示指标名次；`screen.py:render_report` 已有实际指标、范围与排除依据未充分呈现 | S1 |
| 本次更新成功了吗，现在看的是什么？ | `services.py:get_peer_discover` 只选择完整榜；页面未完整呈现空结果提示、最新partial/失败和旧榜关系 | S1 |
| 看完公司怎么继续原来的比较？ | 公司返回固定首页；历史任务没有按自身参照恢复结果上下文 | S1 |
| 加入观察后怎么更新？ | 首页更新按钮禁用，watch 类任务未实现；重扫同业不能替代固定关注更新 | S2 |
| 到底改了什么，能否确认已阅？ | 首页有变化摘要，详情缺少已阅值与当前值的逐项对照，却提供已阅操作 | S2；S1不得因快捷加入推进ack |
| 移动端是否真能独立使用？ | 现有390px浏览器脚本证明部分点击/保存链路，不证明可理解性、真机或生产认证 | S3；各切片同步测浏览器 |

**顺序：S1「一次扫描看得懂」→ S2「加入以后能继续研究」→ S3「真机与生产使用验收」。**沿用S1/S2/S3名称，但替代1.0版本的从零建环境、再建worker的执行顺序；不重做已有基础。每次只实现一个可用切片，详细范围、反例和退出条件由路线图维护。

产品成功不是显示更多股票或生成更高分，而是所有者能独立说明：研究谁、依据是什么、资料有什么缺口、下一步核查什么。已有报告的信息组织优先复用；只改善展示不改原前三、资格或公式。
...
首次：登录 → 空清单显示可点击的“开始同业研究” → 选择允许参照并扫描 → 看前三、参照及依据 → 加入观察。正常用户流程不要求手动导入、命令行或阅读README；已有快照仍可直接查看。尚不支持的参照或缺少更新配置需解释原因，不做无提示空页。服务器配置不由Web端/移动端用户反复填写。
...
日期不一致时显示“各公司数据日不同”，由卡片给出日期；不能用一个最新日期替所有公司背书。卡片优先级：数据错误单独提示；事实未阅变化；研究中；观察。卡片显示上次理由与下一步核查；未填写明确留空，不生成投资结论。已暂停的公司不在默认提醒区或待阅计数中，可展开并恢复；不删除个人记录。任务历史放次级区域，不挤占研究任务。S1期间若watch未接入，在首页可见说明及“前往同业扫描”入口，不能只用禁用按钮的tooltip解释；S2才启用固定关注更新。
...
本次尝试：数据日 / 完整、部分完成或失败
当前完整榜：扫描日期 / 行业 / 范围限制（若为旧榜明确标注）
参与与排除：枚举数 / 截取数 / 合格数 / 原因

示例公司丙       同业研究次序 1（不是买入建议）
逐年 ROE：年度和值 / 年度和值 / 年度和值
ROE 均值 ……%    PB ……倍（各自数据日）
与参照：PB差值 / ROE均值差（百分点），注明采用年度
我的状态：尚未研究
[查看依据] [加入观察]
...
前三保持算法原结果；个人标记“暂不研究”不删除其算法名次，也不把第四名伪装成第三名。参照不在前三或不合格仍单列，写明原因；已在前三时不重复卡片。完整排名在二级区域以紧凑卡片/可换行列表呈现，不强迫移动端浏览器横向滚动宽表。

卡片以实际PB、逐年ROE和均值为主；PB/ROE名次及平均名次为可展开的计算依据，不再用不解释的“综合”作主信息。显示个人状态，直接“加入观察”幂等且ack为空；已有记录显示“查看我的研究”，不覆盖理由。对比取同一已展示完整run，数据日/采用年度不同需显式提示，缺失不补零、不从另一份run拼出完整比较。

展开区域复用 `render_report`、`candidate_review_sections` 及现有快照信息组织：行业标签、来源、枚举/截取/合格数、50家上限与大市值偏向、缺失和已知业务排除。可以共享纯数据整理函数，不复制排序算法，也不必直接嵌入整份Markdown或暴露私人快照下载。
...
本次变化（已阅资料日期 → 当前资料日期）
  PB：旧值 → 新值，分别标注估值日
  年报：年度/公告日/版本/逐年ROE，旧值 → 新值
  若是成员/规则/缺失变化，分别说明；仅核查时间变化不算财务事实变化
...
“保存记录”和“已阅”严格分开。详情必须先实际展示已阅基准→当前可用事实的变化，再允许确认；无基准时显示首次待阅及完整当前事实。不能只有“年报有变化”摘要就允许确认变化，不能只拿latest的编号调用ack。打开详情不自动推进已阅基准；修改备注不自动将变化标成已阅。退出有未保存编辑时提示保存/放弃/继续编辑。未提交草稿只保留在当前页面内；浏览器强制回收可能丢失草稿，不能显示“已保存”。本轮不做离线草稿同步。

**任务页：**受理后显示“服务器已接收，可稍后回来”。状态用“排队中/更新中/完成/部分完成/失败/已中断”，不直接展示 `succeeded / partial`。只有worker持久化了真实逐公司计数才显示 `已处理7/18`；仅有任务级进度时只显示实际阶段，不将 `1/1` 冒充全部证券成功，不估算完成时间。终态提供“查看本次结果/诊断”和适用的“重试”，见2.6。顶部小任务提示使离开任务页后仍能找到未完成任务。

**设置页：**优先展示数据是否可用、最新成功更新与失败摘要、阻止更新的原因和可行动提示；正常页面不以绝对数据目录、内部ID为主。保留登录身份和退出；诊断不得暴露凭据或提供网页shell。
...
工具安排研究顺序，不计算内在价值或买卖信号。以下提示是人工核查问题，不是假定已发现风险，也不改变任何资格或名次：

| 展示的事实 | 必须解释的边界 / 下一步核查 |
|---|---|
| 较低PB | 不等于低估；需核查资产减值、资产质量与盈利持续性 |
| 较高ROE | 不等于未来回报更高；需核查杠杆、净资产变化和一次性损益 |
| 三年均值与逐年值 | 同时显示年度，避免均值遮盖下行、亏损年度或周期高点 |
| 同业标签 | TuShare粗行业不是业务可比性证明；主营构成、资产结构和周期需人工确认 |
| 前三与研究次序 | 仅为当前有限样本内两项名次的平均，不是推荐分；小分差不代表显著价值差 |
| 数据完整/已阅 | 只说明覆盖字段核验或已看过，不表示风险已排除、研究完成或可交易 |

来源区域展示现有端点/资料来源、年度、公告日、版本依据、取得/核查时间和缺口；无原文链接不伪造公告URL。外部笔记与合法来源链接经校验后仅浏览器打开。现有reason/next_check字段足以记录“为什么继续看”和“怎样证伪”，不新增评分模型、投资结论字段或自动研究平台。

### 2.6 结果状态与导航合同

| 情形 | 页面必须显示 | 可用操作 |
|---|---|---|
| 尚未扫描/无历史完整榜 | 明确空状态及允许的参照范围，不渲染无提示空列表 | 选择参照、提交扫描 |
| 排队或更新中 | 本次目标日、真实阶段；如有旧榜，单独写明旧日期 | 离开页面、回看已受理任务 |
| 完整榜且有候选 | 前三＋参照、依据与范围，不因个人状态重排 | 查看依据、加入观察、展开完整比较 |
| 零合格/无池外合格对象 | 具体业务排除/覆盖缺口；沿用既有 `discovery_complete` 与health，不为UI改成成功 | 查看排除原因、改选允许参照 |
| partial，有旧完整榜 | “本次部分完成”＋未更新范围；“当前展示上次完整榜”及其日期 | 查看本次诊断、旧榜；明确重试 |
| partial，无完整榜 | 本次已得事实与数据缺口，不放入正式前三区域 | 查看诊断、明确重试 |
| 失败/中断，无run | 短错误原因、旧资料可用性；不能由空白暗示无候选 | 回看旧结果、明确重试 |
| 快照损坏/日期倒退 | 异常与可用旧资料日期分开，不悄悄回退 | 查看历史可用资料；无可用时明确说明 |

榜单run、当前尝试job与其result_run_id分别保留；任务“查看本次结果”打开该任务的参照及指定结果/诊断，而不是当时页面选中的另一家公司或后来最新run。新扫描/重试由本人点击、产生新request_id并重新证明目标日；不暗中更改原任务的冻结范围和日期。若新范围/目标日改变，提交前明确提示。

公司返回恢复来源页、参照、展示run与列表位置；从首页进入则回首页。底部导航选中态一致；浏览器返回/刷新需定义并测试，无法恢复临时上下文时回安全且明确的入口，不重放任务或写操作。来源上下文只用于每页导航，不放宽鉴权，不持久化私人事实到浏览器存储。
...
└── docker/ + docker-compose.yml + deploy.sh   现有Nginx/Compose部署资产
```
...
以上为复用边界，不是要求重做现有模块。`9a78ff3` 的worker已显式重新核查财务、记录成功checked_at并在读取结束后设置捕获时刻；CLI仍保留原缓存语义。七天复用是允许的优化，不要求先建立缓存层；未知核查时刻不能当作近期核查。
...
加入关注与确认已阅是两个动作：首次加入ack为空，即使已经打开详情也不自动推进；只有用户显式确认实际展示的可用资料才设置初始ack。`mark_seen`中的displayed_run_id由服务端页面呈现时捕获，不在点击时重新查latest。服务校验证券存在、该行facts_usable、当前授权和revision；旧标签页不能把ack倒退到更早的资料。同一run重复已阅返回当前值，不产生假更新。已阅不等于风险核查完成。
...
沿用现有 `docker-compose.yml`、`Dockerfile`、`docker/nginx-stock.conf` 与 `deploy.sh`；实际启动及失败边界见[项目说明](../README.md)。不新增Caddy/systemd并行部署，不以容器数量作为产品验收目标。

```text
同一镜像 → Web容器：OAuth配置；本地HTTP/受控代理网络
         → worker容器：TuShare配置；独立串行任务进程
共享持久化工作区：workspace.sqlite3 / snapshots / worker.lock
tracker输入：只读挂载所需参照、已证明日历和可选原库
```

角色凭据仅注入所需进程；`.env`、`.worker.env`及数据目录不得提交或进入镜像。文件权限由部署者按最小权限设置，不声称仅拆环境文件就提供严格OS级隔离。生产不用--reload。tracker 路径只授予必要读取权限，不能chmod整个主目录；WAL原库须验证sidecar可读性，不能以immutable=1冒充稳定文件。缺参照/日历只阻止相关更新，不妨碍查看已有工作区。

Nginx保留HTTPS/WSS与OAuth回调，Web端口不直接开放公网；验证Host、Origin、WS握手和重连。保持单个Web进程与独立worker，ASGI包装器透传startup/shutdown。[F2]
...
只维护和校验现有部署资产；提交/推送不等于部署授权。重建容器、重载代理、改权限或访问真实数据仍须所有者明确授权。
...
四个入口已存在：`make setup`、`make demo`、`make check`、`make test-mobile`。各切片维护它们，不再要求记住相邻项目虚拟环境路径。本文之外不新建多份spec/plan；必要决策写进这两份维护文档。
...
完成以用户独立执行为准：不看README、不用CLI即可扫描，说明前三/参照的实际差异与缺口，加入观察；日后能更新固定关注、看到已阅→当前变化、继续上次核查并保存。S1验证扫描可理解，S2验证持续研究，S3验证本人真实终端与生产链路；具体退出条件见路线图。通过单元测试、匿名HTTP 200或容器运行均不等于产品验收。
...
暂不更换Flet。S1后若仍有可复现的首屏、中文输入、浏览器导航或重连阻断，先测量定位；记录设备/浏览器、冷热启动、资源网络、复现操作与失败频次，再决定局部修复还是替换展示层。架构变更须单独批准，保留算法、服务和工作区，不并行养两套前端。没有证据不承诺加载时间改善，不把美化、AI总结、行情图或合并容器当作上述闭环的替代。

## 附录 A：最小 DDL（设计合同，实施时由初始化函数创建）

本附录保留目标存储合同，不代表与当前 `workspace.py` 每个字段默认值完全一致；实施先核对schema/version及兼容性，不能为页面改造重建或覆盖已有库。差异若涉及迁移须单独列出、验证备份并获授权。所有写入先在Python层校验；下列SQL不包含真实数据或用户配置。日志模式在初始化前按4.5核验；显式选择DELETE的新建环境仅替换journal_mode这一行，其余合同不变。以下建表及user_version需原子执行；错误后整体回滚。
