...
| `services.py` / `auth.py` / `app.py` | 离线查询与个人记录、鉴权、Flet Web/ASGI |
| `test_workspace.py` / `test_services.py` / `test_app.py` | 离线工作区、服务和 Web 安全测试 |
...
- **S2 未实现**：页面不提交更新任务；无独立 worker，更新按钮为禁用提示。CLI 的 `--refresh` 仍是显式联网操作。
